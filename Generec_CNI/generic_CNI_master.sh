...
drug_name=$1
node_list=${drug_name}_pub_nodelist
edgelist=${drug_name}_pub_edgelist
authorpub=${drug_name}_scopus_pub_auth


# calculate the citations and PIr scores
psql -d elsevier -v node_list=$node_list -v edge_list=$edgelist -f pub_pir_scores.sql
