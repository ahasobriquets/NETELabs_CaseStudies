...
\added[id=gc,remark={expanded discussion of generalizability as per editor request}]{Here, we started with the literature directly cited in the approval process for five therapeutics, which are subject to multiple layers of independent assessment, i.e., patent awards, clinical trials, and FDA reviews that in turn contribute to the relevant literature. However, the only requirement of the current method is a starting set of articles. Any question premised upon the citation record and defined by a nucleus of articles can be addressed with this approach. The method is applicable to any other therapeutic or set of therapeutics, but also to medical devices, diagnostics, standards development, and research discoveries and protocols such as induced pluripotent (IPS) cells  or CRISPR-Cas9. Furthermore, the method is generalizable to sets of articles defined by groups of authors (e.g., consortia, programs), by institutions or departments, or by grant portfolios. The application of our method to the bibliographic record of a department, for example, would provide novel, network-based metrics by which to assess collaboration, productivity, translational impact, etc. No matter the application, the method is implicitly time-dependent, allowing one to make historical comparisons and to set future goals. Importantly, our improvements to the method as presented here allow for the analysis across sets of articles in addition to within, supporting higher level interrogations of the crosstalk and collaborations that connect groups.}
