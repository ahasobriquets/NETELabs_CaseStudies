# Work with final_results of 5/5/2017 from Eric.
...
final_authors <- read.csv("pub_auth.csv", stringsAsFactors=FALSE)

t1 <- data.frame(union(alem_m3$citing_sid,alem_m3$cited_sid),stringsAsFactors=FALSE)
colnames(t1) <- ("doc_sid")
alem_m4 <- merge(t1,final_authors,by.x="doc_sid",by.y="PubSID",all.x=TRUE)
colnames(alem_m4) <- c("doc_sid","auth_sid","FullName","LastName","FirstName","Initials","DocCount")

t1 <- data.frame(union(imat_m3$citing_sid,imat_m3$cited_sid),stringsAsFactors=FALSE)
colnames(t1) <- ("doc_sid")
imat_m4 <- merge(t1,final_authors,by.x="doc_sid",by.y="PubSID",all.x=TRUE)
colnames(imat_m4) <- c("doc_sid","auth_sid","FullName","LastName","FirstName","Initials","DocCount")

t1 <- data.frame(union(nela_m3$citing_sid,nela_m3$cited_sid),stringsAsFactors=FALSE)
colnames(t1) <- ("doc_sid")
nela_m4 <- merge(t1,final_authors,by.x="doc_sid",by.y="PubSID",all.x=TRUE)
colnames(nela_m4) <- c("doc_sid","auth_sid","FullName","LastName","FirstName","Initials","DocCount")

t1 <- data.frame(union(ramu_m3$citing_sid,ramu_m3$cited_sid),stringsAsFactors=FALSE)
colnames(t1) <- ("doc_sid")
ramu_m4 <- merge(t1,final_authors,by.x="doc_sid",by.y="PubSID",all.x=TRUE)
colnames(ramu_m4) <- c("doc_sid","auth_sid","FullName","LastName","FirstName","Initials","DocCount")

t1 <- data.frame(union(suni_m3$citing_sid,suni_m3$cited_sid),stringsAsFactors=FALSE)
colnames(t1) <- ("doc_sid")
